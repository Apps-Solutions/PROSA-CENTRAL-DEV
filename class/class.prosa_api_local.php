...
class prosaApi extends api{
	...
	protected function check_token(){
		...
		if ( $result === FALSE ) {
			...
			throw new Exception('Database error. Check Token');
		} 
	...
	protected function set_apple_token( ){
		...
		if ( $this->User->user != '' ){
			...
			if ( !$result ){ 
				$this->set_error( 'An error occured while querying the DB for the token. ', LOG_DB_ERR, 3 );
				throw new Exception('Database Error. Set_apple_token' );
			} 
			if (count($result) < 1 ) {
		...
		global $obj_bd;
		 if ($this->User->user != ''){
			...
			if ( $result === FALSE ){  
				...
				throw new Exception('Database Error. get_alerts' );
			} else {
				$alerts = array();
...
//			}
			...
			if ( $result === FALSE ){  
				$this->set_error( 'An error occured while querying the DB for the services. ', LOG_DB_ERR, 3 ); 
				throw new Exception('Database Error. Get_services');
			} else {
				$services = array();
				foreach ($result as $k => $se) {
					$srv = array();
			...
			/**/if ( $this->User->profile != 1 ){
				...
				if ( $resp ){
					$this->set_error('An error occured while querying the DB for permitted services.', LOG_API_ERR, 3);
	           		throw new Exception('Database Error. Get_indicator');
	...
	protected function get_clients(){
		...
		//if ($this->User->user != ''){
			...
			if ( $result === FALSE ){  
				...
				throw new Exception('Database Error. Get_clients');
			} else {
				$clients = array();
				foreach ($result as $k => $se) {
					$srv = array();
