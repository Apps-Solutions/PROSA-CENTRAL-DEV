 ...
 * Multiserv Class
 * 
 * "Servicio ofrecido a los Clientes para aquellas TPVs bajo el esquema Front End 
 * cuyo protocolo de comunicación es a través de Internet
 * La red lógica adquirente para distinguirlo es: PROI" 
 * 
 */
class AdminMultiserv extends Service {
	
	function __construct( $id_client = 0) {
		
		parent::__construct();
		
		$this->class 	= "AdminMultiserv";
		$this->service 	= "Multiserv";
		$this->code 	= "s9";
	...
	public function load_service(){
		...
		$this->indicators[0]['source'] = "Adquirente";
			...
			$resp = $this->insert_old_last_total($id_service=9, $value);
	...
	private function set_service_totals(){
		...
		$query =  " SELECT DIA, SUM(ACCEPTED) AS ACCEPTED, SUM(REJECTED) AS REJECTED, SUM(TOTAL) AS TOTAL "
					. "  FROM ( "
						...
						. " WHERE  LN_COMER = 'PROI' AND DIA = :dia "
