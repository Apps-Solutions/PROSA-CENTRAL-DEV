...
class ATM extends Service {
	...
	private function set_sellcom_service_totals()
	{
		...
		$query = "SELECT MAX(idpra_charts_ATM) AS id FROM " . PFX_MAIN_DB . "charts_atm WHERE pca_type='emisor_atm' ";
		$query2 = "SELECT MAX(idpra_charts_ATM) AS id FROM " . PFX_MAIN_DB . "charts_atm WHERE pca_type='adquirente_atm' ";
		...
		if($result !== FALSE && $result2 !== FALSE)
		{
			if (count($result) > 0 && count($result2) > 0) 
			{
				...
				$query = " SELECT * FROM " . PFX_MAIN_DB . "charts_atm WHERE idpra_charts_ATM=" . $this->id_service;
				$query2 = " SELECT * FROM " . PFX_MAIN_DB . "charts_atm WHERE idpra_charts_ATM=" . $this->id_service2;
				...
				if ($result !== FALSE && $result2 !== FALSE) 
				{
					if (count($result) > 0 && count($result2) > 0) 
					{
						...
						$this->indicators[0]['total_transactions'] = $total['pca_total_acepted'] + $total['pca_total_rejected'];
						$this->indicators[0]['total_accepted'] = $total['pca_total_acepted'];
						$this->indicators[0]['total_rejected'] = $total['pca_total_rejected'];

						$this->indicators[1]['total_transactions'] = $total2['pca_total_acepted'] + $total2['pca_total_rejected'];
						$this->indicators[1]['total_accepted'] = $total2['pca_total_acepted'];
						$this->indicators[1]['total_rejected'] = $total2['pca_total_rejected'];

						$datos = $total['pca_top_5_rejected'];
						$datos2 = $total2['pca_top_5_rejected'];
