...
class PREA extends Service {
	...
	private function set_sellcom_service_totals()
	{
		...
		$query = "SELECT MAX(idpra_charts) AS id FROM " . PFX_MAIN_DB . "charts WHERE pcs_type='emisor_pos' AND pcs_se_id_service=2";
		$query2 = "SELECT MAX(idpra_charts) AS id FROM " . PFX_MAIN_DB . "charts WHERE pcs_type='emisor_atm' AND pcs_se_id_service=2";
			...
			if (count($result) > 0 && count($result2) > 0) 
			{
				...
				if ($result !== FALSE && $result2 !== FALSE) 
				{
					if (count($result) > 0 && count($result2) > 0) 
					{
						...
						$this->indicators[1]['total_transactions'] = $total2['pcs_total_acepted'] + $total2['pcprea_total_rejected'];
