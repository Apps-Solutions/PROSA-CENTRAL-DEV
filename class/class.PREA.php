...
class PREA extends Service {
	...
	private function set_sellcom_service_totals()
	{
		...
		$query = "SELECT MAX(idpra_charts_PREA) AS id FROM " . PFX_MAIN_DB . "charts_prea WHERE pcprea_type='emisor_pos' ";
		$query2 = "SELECT MAX(idpra_charts_PREA) AS id FROM " . PFX_MAIN_DB . "charts_prea WHERE pcprea_type='emisor_atm' ";
		...
		if($result !== FALSE && $result2 !== FALSE)
		{
			if (count($result) > 0 && count($result2) > 0) 
			{
				...
				$query = " SELECT * FROM " . PFX_MAIN_DB . "charts_prea WHERE idpra_charts_PREA=" . $this->id_service;
				$query2 = " SELECT * FROM " . PFX_MAIN_DB . "charts_prea WHERE idpra_charts_PREA=" . $this->id_service2;
				...
				if ($result !== FALSE && $result2 !== FALSE) 
				{
					if (count($result) > 0 && count($result2) > 0) 
					{
						...
						$this->indicators[0]['total_transactions'] = $total['pcprea_total_acepted'] + $total['pcprea_total_rejected'];
						$this->indicators[0]['total_accepted'] = $total['pcprea_total_acepted'];
						$this->indicators[0]['total_rejected'] = $total['pcprea_total_rejected'];

						$this->indicators[1]['total_transactions'] = $total2['pcprea_total_acepted'] + $total2['pcprea_total_rejected'];
						$this->indicators[1]['total_accepted'] = $total2['pcprea_total_acepted'];
						$this->indicators[1]['total_rejected'] = $total2['pcprea_total_rejected'];

						$datos = $total['pcprea_top_5_rejected'];
						$datos2 = $total2['pcprea_top_5_rejected'];
