...
class Client extends Object {
	...
	function __construct( $id_client, $users = TRUE, $services = TRUE ) {
		...
		if ( is_numeric($id_client) && $id_client > 0 ){
			...
			if ( !$result ){
				...
				$this->set_error( "No se encontr� el registro. ", ERR_DB_NOT_FOUND, 1 );
				$this->clean();
			} else {
				$info = $result[0];
	...
	private function get_services(){
		if ( $this->id_client > 0 ){
			...
			if ( $result !== FALSE ){
				...
				$this->set_error( "Ocurri� un error al consultar los servicios en la BBDD. ", ERR_DB_QRY, 1 );
			} 
		}
	}
	...
	private function validate(){ 
		if ( !$this->client != '' ){
			$this->set_error( 'Nombre de Cliente vac�o. ', ERR_VAL_EMPTY );
			return FALSE;
		} 
		if ( !$this->code != '' ){
			$this->set_error( 'C�digo de Cliente vac�o. ', ERR_VAL_EMPTY );
			return FALSE;
		} 
		return TRUE;  
	}
		...
		if ( $Session->is_admin() ){
			if ( $this->validate() ){ 
				...
				if ( !$resp ){ 
					$errs = $this->db->error; 
					// $errs[(count($errs)-1)] 
					$this->set_error( "Ocurri� un error al intentar guardar la infomraci�n. (save)" , ERR_DB_EXEC, 2);
					return FALSE;
				} 
				else return TRUE;
			}
		} else {
			$this->set_error( 'Acci�n restringida. ', SES_RESTRICTED_ACCESS );
			return FALSE; 
		}
	}
	...
	public function get_client_services_form(){
		...
		if ( $Session->is_admin() ){
			...
			$this->set_error("Acci�n restringida get_client_services_form ", SES_RESTRICTED_ACTION, 3 );
			ob_start();
	...
	public function get_client_users_services_table( $user ){
		...
		if ( $Session->is_admin()){ 
			if ( $this->has_user( $user ) ){ 
				if ( count($this->services) > 0 ){ 
					...
					if ( $result ){
						...
						$this->set_error("Ocurri� un error al cargar la informaci�n.",ERR_DB_QRY);
						return FALSE;
					} 
				} else {
			...
			$this->set_error("Acci�n restringida get_client_users_services_table ", SES_RESTRICTED_ACTION, 3 );
			ob_start();
			require DIRECTORY_VIEWS . "base/403.php"; 
			$response .= ob_get_clean(); 
			return $response;   
